...
export default function AdverseEventsTable({ data }) {
  ...
  const filterData = (items) => {
    return items.filter((item) => {
      const nameMatch = item.name
        .toLowerCase()
        .includes(filters.conceptName.toLowerCase());
      const meddraMatch = item.id.toString().includes(filters.meddraId);
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex gap-4">
        <Input
          ...
          onChange={(e) =>
            setFilters({ ...filters, conceptName: e.target.value })
          }
          className="max-w-sm"
        />
        <Input
          ...
          <TableBody>
            {currentData.map((row, index) => (
              <TableRow
                ...
                onClick={() => router.push(`/adverse/${row.id}`)}
              >
