...
export default function AdverseReaction() {
  ...
  useEffect(() => {
    setLoading(true);
    getDrugsByAdverseReaction(params.id).then((res) => {
      setDrugs(res.drugs);
      let term_name = res.adverse_reaction.concept_name;
      setAdverseReaction(term_name);
      setLoading(false);
      getDefiniton(term_name).then((res) => {
        setDesc(res);
      });
    });
  }, [params.id]);
                        ...
                        overlay={
                          <Tooltip>
                            <span>
                              ...
                              Thesaurus using the National Center for Biomedical
                              Ontology's BioPortal
                            </span>
                          </Tooltip>
                        }
                      >
                ...
                {drugs.map((item) => (
                  <li key={item.drug_concept_ids}>
                    {item.ingredients.split(",").length === 1 ? (
                      <Link to={"/drugs/" + item.drug_concept_ids}>
                        {item.ingredients}
                      </Link>
                    ) : (
                      item.ingredients.split(",").map((ind_drug, index) => (
                        <React.Fragment
                          key={item.drug_concept_ids + "-" + index}
                        >
                          <Link
                            to={
                              "/drugs/" +
                              item.drug_concept_ids.split(", ")[index]
                            }
                          >
                            {ind_drug.trim()}
                          </Link>
